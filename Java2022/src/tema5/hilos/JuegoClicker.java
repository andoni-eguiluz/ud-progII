...
public class JuegoClicker {
	...
	private ArrayList<Clicker> listaClickers;  // Lista de clickers activos
	...
	private void anyadirNuevoClicker() {
		if (random.nextDouble() < probNuevoClicker) {  // Posibilidad de clicker cada fotograma.
			...
			for (int i=0; i<PROB_CLICKERS.length; i++) {
				if (queClicker <= PROB_CLICKERS[i]) {
					...
					Clicker elto = new Clicker( tiempoNuevoClicker, CLICKERS[i], xRandom, yRandom, anchuraClicker, alturaClicker, 0.0, 1.0f );
		...
		private void asignaEventos( Clicker elto ) {
	...
	private void quitarClickersDesaparecidos() {
		...
		Iterator<Clicker> it = listaClickers.iterator();
		while (it.hasNext()) {
			Clicker clicker = it.next();
			if (clicker.desaparecido()) {
	...
	// Click de elemento
	private void clickEnElemento( Clicker elto, Point pto ) {
	 ...
	 */
	private static class Clicker extends JLabel {
		...
		static {
			try {
	    		URL imgURL = Clicker.class.getResource( "img/ok.png" ).toURI().toURL();
				IMAGEN_CLICK_CORRECTO = ImageIO.read(imgURL);
	    		imgURL = Clicker.class.getResource( "img/wrong.png" ).toURI().toURL();
		 ...
		 */
		public Clicker( long cicloDibujado, String nombreImagenObjeto, int x, int y, int anchura, int altura, double rotacion, float opacidad ) {
		...
		public void setImagen( String nomImagenObjeto ) {
			if (mapaImagenes.containsKey(nomImagenObjeto)) {  // Si ya se ha cargado antes, se reutiliza
		        ...
		        try {
					...
					try {
			    		imgURL = Clicker.class.getResource( nomImagenObjeto ).toURI().toURL();
						imagenObjeto = ImageIO.read(imgURL);
					} catch (Exception e2) {  
